---
date: 2025-06-10
title: CMU - 36-705 - Intermediate Statistics - Lesson 01
...
* Change of variables is reviewed for functions of multiple random variables:
  ...
  * For a transformed variable $(Z=g(X,Y))$, start from the cumulative distribution function:
    $$
    F_Z(z)=P(g(X,Y)\le z)
    $$
...
* Independence is defined formally:
  ...
  * (X) and (Y) are independent if
    ...
    $$
    for all events (A,B).
  * Equivalently, the joint density factors:
    $$
    p(x,y)=p_X(x)p_Y(y)
    $$
...
* The distinction between *random variables* and *parameters* is emphasized:
  ...
  * [The random variable is the object being probabilistically described.]{.marked}
  * [Parameters such as $(\mu), (\sigma^2), (n), or (\theta)$ index a family of distributions and are treated as fixed, though often unknown.]{.marked}
...
* For the sample mean $\bar X$:
  ...
  * If $X_1,\dots,X_n$ are independent and identically distributed with mean $\mu$ and variance $\sigma^2$, then:
    $$
...
* In the special normal case:
  ...
  * If the original observations are normally distributed, then (\bar X) is also normally distributed.
...
* [Chebyshev’s inequality](https://en.wikipedia.org/wiki/Chebyshev%27s_inequality) is derived from Markov’s inequality:
  ...
  * This yields a bound involving the variance:
    $$
...
:::
::: {.sl-fig}
![Lecture Notes 1](https://www.stat.cmu.edu/~larry/=stat705/Lecture1.pdf)
:::
::::