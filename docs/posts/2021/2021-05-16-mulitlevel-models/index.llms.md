...
[![pooled-plate-model](pooled-plate-model.png)](pooled-plate-model.png "pooled-plate-model")
...
[![unpooled-plate-model](unpooled-plate-model.png)](unpooled-plate-model.png "unpooled-plate-model")

unpooled-plate-model

Model radon in each county independently.

y_i=\alpha\_{j\[i\]} + \beta x\_{j\[i\]} + \epsilon_i
...
[![partially-pooled-plate-model](partialy-pooled-plate-model.png)](partialy-pooled-plate-model.png "partially-pooled-plate-model")
...
recall: y_i=\alpha\_{j\[i\]} + \beta x\_{j\[i\]} + \epsilon_i

we now want to estimate an county level using a weighed average between the county level and the over all level average. \hat{\alpha}\_j \approx \frac{(n_j/\sigma_y^2)\bar{y}\_j + (1/\sigma\_{\alpha}^2)\bar{y}}{(n_j/\sigma_y^2) + (1/\sigma\_{\alpha}^2)}
...
\alpha\_{j\[i\]} \sim N(\mu\_{\alpha}, \sigma\_{\alpha}^2)
...
[![](varying-slopes-plate-model.png)](varying-slopes-plate-model.png)
...
y_i = \alpha + \beta\_{j\[i\]} x\_{i} + \epsilon_i
...
[![varying-intercepts-and-slopes-plate-model](varying-intercepts-and-slopes-plate-model.png)](varying-intercepts-and-slopes-plate-model.png "varying-intercepts-and-slopes-plate-model")
...
y_i = \alpha\_{j\[i\]} + \beta\_{j\[i\]} x\_{i} + \epsilon_i
...
- x (0 for basement 1 for 1st) floor of the house.
- y log radon measurement
- \alpha\_{ji} intercept parameter (corresponding to a base radon level) partially pooled across all counties.
- \beta\_{ji} gradient parameter (corresponding to drop of radon level between floors) partially pooled across all counties.
- \epsilon_i error term considered at the house level.

## Hierarchical Intercepts and Slopes

[![hierarchical-intercepts-model](hierarchical-intercepts-model.png)](hierarchical-intercepts-model.png "hierarchical-intercepts-model")
...
Group-level predictors also serve to reduce group-level variation \sigma\_{\alpha}. An important implication of this is that the group-level estimate induces stronger pooling.

## Correlation amongst levels

[![correlations-among-levels-plate-model](correlations-among-levels-plate-model.png)](correlations-among-levels-plate-model.png "correlations-among-levels-plate-model")
...
@online{bochman2021,
  ...
  date = {2021-05-16},
  url = {https://orenbochman.github.io/posts/2021/2021-05-16-mulitlevel-models/},
  langid = {en}
}
```
...
Bochman, Oren. 2021. “Multilevel Models.” May 16. <https://orenbochman.github.io/posts/2021/2021-05-16-mulitlevel-models/>.
