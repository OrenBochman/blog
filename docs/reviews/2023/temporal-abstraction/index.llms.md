...
Ever since I saw ([White 2022](#ref-Martha2022SubTasks)) the video lecture on [subtasks](#fig-subtasks) by Martha White about learning tasks in parallel. However the video does not address the elephant in the room - how to discover the options.
...
This is a hefty paper 70 pages with 8 algorithms many figures and citations from research spanning thirty years. It is filled to the brim with fascinating concepts that are developed by the authors but builds on lots of work by earlier researchers. It may seem to cover a niche topic but (c.f. [Machado 2024](#ref-Machado2024Cohere), time 773) makes an eloquent argument that this paper deals with a fundamental question of where options come and if we put aside the jargon for a second we are trying to capture a form of intelingence that includes elements of generalization, planning, problem solving, learning at a level much closer what we are familiar with. And these familiar forms of mental abstractions much harder to consider in the context of Supervised or Unsupervised learning which lack the ineraction with the environment that is the hallmark of reinforcement learning.
...
In ([**reinforcement_2024?**](#ref-reinforcement_2024)) Marlos C. Machado, has given a talk that explains many of the complex ideas within this paper. This talk is available on [YouTube](#fig-option-discovery).
...
> >
> > — ([Machado et al. 2023](#ref-machado2023temporal))
...
In ([Precup 2017](#ref-Doina2017DeepHack)) Doina precup gives a talk on temporal abstraction in reinforcement learning. This talk covers both the introduction and the background material on options and is on [YouTube](#fig-option-framework).
...
- Introduces the options framework ([Sutton et al. 1999](#ref-Sutton1999BetweenMA)), ([Precup and Sutton 2000](#ref-precup2000temporal)), defining its components (initiation set, policy, termination condition), execution models, and potential benefits.

An option \omega \in \Omega is a 3-tuple

\omega = \<I\_\omega , \pi\_\omega , \beta\_\omega \> \qquad \tag{1}

- where
  - I\_\omega ⊆ S the options’s initiation set,
  - \pi\_\omega : \mathcal{S} \times \mathcal{A} \rightarrow \[0, 1\] the option’s policy, such that \sum_a \pi\_\omega (·, a) = 1, and
  - \beta\_\omega : \mathcal{S} \rightarrow \[0, 1\] the option’s termination condition [^1]

## A Framework for Option Discovery from Representation Learning

[![](./fig_1.png)](./fig_1.png "Figure 4: Representation-driven Option Discovery (ROD) cycle [@Machado2019EfficientEI]. The option discovery algorithms discussed in this paper can be seen as instantiating this cycle. The incoming arrow to Collect samples depicts the start of the process. The arrow from Define option to Option set highlights the output generated by the ROD cycle. Note that other generated artifacts can also be used by the agent outside the ROD cycle, such as the learned representation.")

Figure 4: Representation-driven Option Discovery (ROD) cycle ([Machado 2019](#ref-Machado2019EfficientEI)). The option discovery algorithms discussed in this paper can be seen as instantiating this cycle. The incoming arrow to Collect samples depicts the start of the process. The arrow from Define option to Option set highlights the output generated by the ROD cycle. Note that other generated artifacts can also be used by the agent outside the ROD cycle, such as the learned representation.
...
- In ([Machado et al. 2023, sec. 4.1](#ref-machado2023temporal)) Defines the SR in the tabular setting, explaining its ability to capture environment dynamics by encoding expected future state visitation, as shown in Equation 7 and [the figure](#fig-3).

\Psi\_{\pi}(s,s') = \mathbb{E}\_{\pi,p} \sum\_{t=0}^\infty γ^t \mathbb{1}\_{\\S_t=s'}\\ \| {S_0 = s } \qquad \tag{2}

[![](./fig_4.png)](./fig_4.png "Figure 6: First three PVFs in the (a) four-room domain. Gray squares represent walls and white squares represent accessible states. Four actions are available: up, down, right, and left. The transitions are deterministic and the agent is not allowed to move into a wall. (b-d) These plots depict the first, second, and third eigenvectors associated with each state. The axes are rotated for clarity. The bottom left corner of the four-room domain is the state closer to the reade")

Figure 6: First three PVFs in the (a) four-room domain. Gray squares represent walls and white squares represent accessible states. Four actions are available: up, down, right, and left. The transitions are deterministic and the agent is not allowed to move into a wall. (b-d) These plots depict the first, second, and third eigenvectors associated with each state. The axes are rotated for clarity. The bottom left corner of the four-room domain is the state closer to the reade

- Discusses the estimation of the SR with temporal-difference learning, its connection to general value functions [^2], and its relationship to the transition probability matrix [Equation 9](#eq-9).

\Psi(S_t,j) \leftarrow \hat{\Psi}(S_t, j) + \eta \[\mathbb{1}\_{\\S_t=j\\} + \gamma \hat{\Psi}(S\_{t+1}, j) − \hat{\Psi}(S_t, j)\] \tag{3}

\Psi\_\pi = \sum\_{t=0}^\infty (\gamma P\_\pi)^t = (I-\gamma P\_\pi)^{-1}\qquad \tag{4}
...
- ([Machado et al. 2023, sec. 5](#ref-machado2023temporal)) discusses temporally-extended exploration with options and its potential to enhance exploration in RL.

- ([Machado et al. 2023, sec. 5.1](#ref-machado2023temporal)) introduces eigenoptions, which are options defined by the eigenvectors of the SR. \> “Eigenoptions are options defined by the eigenvectors of the SR.2 Each eigenvector assigns an intrinsic reward to every state in the environment.”
...
- ([Machado et al. 2023, sec. 6](#ref-machado2023temporal))Evaluates eigenoptions and covering options in the context of temporally-extended exploration.
...
[![](./fig_9.png)](./fig_9.png "Figure 7: figure 9")

Figure 7: figure 9

## Iterative Option Discovery with the ROD Cycle

- ([Machado et al. 2023, sec. 7](#ref-machado2023temporal)) introduces Covering Eigenoptions (CEO), a new algorithm that performs multiple iterations of the ROD cycle for option discovery.
...
[![](./alg_1.png)](./alg_1.png "Figure 8: successor representations algorithms")

Figure 8: successor representations algorithms

Next is the covering eigenoptions algorithm:

[![](./alg_2.png)](./alg_2.png "Figure 9: Covering Eigenoptions algorithm")
...
In reinforcement learning, an **option** is a temporally extended course of actions that allows an agent to operate at different levels of abstraction within an environment. Options are a form of temporal abstraction that enables agents to make predictions and execute actions over extended time horizons, providing a way to structure and organize the agent’s behavior. v\_{\pi,\beta}^{c,z}(s) = \mathbb{E}\_{\pi,\beta} \left\[ \sum\_{j=1}^K c(S_j) + \gamma^{K-1} z(S_k) \| S_0 = s \right\] \qquad \text{for all } s \in S

- where
  - v\_{\pi,\beta}^{c,z}(s) is the value function of the option,
  - \pi is the policy,
...
- For planning: you can use eigenvectors of the SR to identify bottleneck, states that are difficult to reach under a random walk, and then use options to guide the agent to those states. c.f. ([Solway et al. 2014](#ref-Solway2014OptimalBH))
...
Ψ\_\pi (s, s') = \mathbb{E}\_{π,p} \[\sum^\infty\_{t=0} γ^t\mathbb{1}\_{S_t = s'} \| S_0 = s \] \qquad \tag{5}
...
Machado, Marlos C. 2019. “Efficient Exploration in Reinforcement Learning Through Time-Based Representations.” In *Revue De Geographie Alpine-Journal of Alpine Research*. PhD thesis.

Machado, Marlos C. 2024. “Representation-Driven Option Discovery in Reinforcement Learning.” In *YouTube*. <https://www.youtube.com/watch?v=m9gYmYEYuIs>.

Machado, Marlos C., Andre Barreto, Doina Precup, and Michael Bowling. 2023. “Temporal Abstraction in Reinforcement Learning with the Successor Representation.” *Journal of Machine Learning Research* 24 (80): 1–69. <http://jmlr.org/papers/v24/21-1213.html>.

Precup, Doina. 2017. “DeepHack.RL: Temporal Abstraction in Reinforcement Learning.” In *YouTube*. <https://www.youtube.com/watch?v=GntIVgNKkCI>.

Precup, Doina, and Richard S. Sutton. 2000. “Temporal Abstraction in Reinforcement Learning.” PhD thesis, University of Massachusetts Amherst.

Solway, Alec, Carlos Diuk, N. Córdova, et al. 2014. “Optimal Behavioral Hierarchy.” *PLoS Computational Biology* 10.

Sutton, Richard S. 1988. “Learning to Predict by the Methods of Temporal Differences.” *Machine Learning* 3: 9–44.

Sutton, Richard S., Doina Precup, and Satinder Singh. 1999. “Between MDPs and Semi-MDPs: A Framework for Temporal Abstraction in Reinforcement Learning.” *Artificial Intelligence* 112 (1): 181–211. https://doi.org/<https://doi.org/10.1016/S0004-3702(99)00052-1>.

White, Martha. 2022. “Developing Reinforcement Learning Agents That Learn Many Subtasks.” In *YouTube*. <https://www.youtube.com/watch?v=GmGL9cVfJG4&t=6s>.
...
Bochman, Oren. 2024. “Temporal Abstraction in Reinforcement Learning with the Successor Representation.” February 10. <https://orenbochman.github.io/reviews/2023/temporal-abstraction/>.

[^1]: the probability that option ω will terminate at a given state.

[^2]: “the SR can be estimated from samples with temporal-difference learning methods ([Sutton 1988](#ref-sutton1988learning)), where the reward function is replaced by the state occupancy”
