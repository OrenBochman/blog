...
- The teachers,  Adam and Martha White, are world class researches researchers.
- The University of Alberta is the strongest university for Reinforcement Learning.
- RL builds on the foundations of Markov Decision Processes, Dynamic Programming, and Monte Carlo methods as well as Machine Learning and Neural Networks.
  - Some of this material actually makes more sense when presented in the context of RL.
- I read reviews by a number of people have taken this specialization. A few have had some background in RL and breezed through the material. Most of the others
  suggested that the material can be challenging particularly the last two courses.
- Some of the assignments appear in RL textbooks, but instead of a few lines of code for the main assignment, in the capstone we get to implement both Parts of the environment, the RL algorithm, the replay buffer, the neural net and the optimizer from scratch. This is a blessing in disguise as it really helps you understand the material. 
- A big chunk of the programming assignments have to handle the environment rather than the RL algorithm. 
- In my opinion many of the assignments are not very interesting - the problems that seem to come from the Course text book are rather trivial.
- If you want to work with RL professionally you will need to do a lot more coding.
  - You should consider some of the extra coding assignments that I have included in the text book.
...
- I don't include the programming assignments in these notes. 
  - I might however include similar algorithms from different environments.
...
- There are lots of good notes on the web on RL.
  - Summeries of the text book
  - Slides for the courses offered by Rishard S. Sutton and Andrew G. Barto
