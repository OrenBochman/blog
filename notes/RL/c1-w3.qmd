...
-   [ ] Recognize that a policy is a distribution over actions for each possible state [\#](#sec-policy-definition)
-   [ ] Describe the similarities and differences between stochastic and deterministic policies [\#](#sec-stochastic-vs-deterministic)
-   [ ] Identify the characteristics of a well-defined policy
-   [ ] Generate examples of valid policies for a given MDP
-   [ ] Describe the roles of state-value and action-value functions in reinforcement learning [\#](#sec-value-functions)
...
-   [x] Understand how Bellman equations relate current and future values
-   [x] Use the Bellman equations to compute value functions the state value function is $v(s)$
:::
...
\begin{align}
v_\pi(B) &= 
     &= 0.25 \times (0.7 \times v(A)) + 0.5 \times (5 + 0.7 \times v(B)) + 0.25 \times (0.7 \times v(D)) \newline
\end{align}
$$ $$
\begin{align}
v_\pi(C) &= 
     &= 0.25 \times (0.7 \times v(A)) + 0.5 \times (0.7 \times v(B)) + 0.25 \times (0.7 \times v(C)) \newline
\end{align}
$$
...
\begin{align}
v_\pi(D) &= 
     &= 0.25 \times (0.7 \times v(B)) + 0.5 \times (0.7 \times v(C)) + 0.25 \times (0.7 \times v(D)) \newline
\end{align}
$$
...
-   [ ] Define an optimal policy [\#](#sec-optimal-policy)
...
Martha asks the question: "How can $\Pi_3$ have better strictly better values than both $\Pi_1$ and $\Pi_2$ in all states if all we did is take the best action in each state from either $\Pi_1$ or $\Pi_2$?"
