...
I found policy gradient methods to be rather complicated at first blush. Eventually, as the finer points sunk in, I developed some bits and pieces of intuition.

**Parametrized policies** are much easier to conceptualize than parametrized value functions. One intuition is that the parameters are weights for the action, and the policy's actions are drawn in proportion to these weights. The **softmax policy** is, in fact, just this. 
My Bayesian-trained intuition for these weights comes from the *categorical distribution* ^[which models an K-sided die based on its historic performance]. Where success is defined as an action that gets my agent closer to its goal. This intuition is just my first-order mental model.

The obvious question that will arise as soon as you deal with some environment is:

>"How can I get some arbitrary feature to participate in the parametrized policy?"

The immediate suspect is *use it to build the weights*. How should that work? The intuitive method for this is a (Bayesian) linear regression. If we tweak this feature, how will the policy give us better returns? Since we want to maximize returns, we should adjust the weights in the direction that provides us with the best returns.
...
One of my goals is to make a satisfactory proof that I properly understand and walk over with ease. 
