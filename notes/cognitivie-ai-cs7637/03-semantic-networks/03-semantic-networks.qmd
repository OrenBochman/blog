...
A large part of this lesson explores solving two types of problems. By using a graph to visually represent the process, we end up with a semantic network. However, it seems that these networks even when implemented in a tool like networks are still unwieldy. 

Also, it seems that humans who can solve these problems can come up with networks. But how can a machine come up with a representation etc given just the picture? 
