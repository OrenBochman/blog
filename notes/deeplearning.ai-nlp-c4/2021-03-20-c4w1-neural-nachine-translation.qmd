...
description: "This week of the Specialization, we will dive into the fundamentals of Neural Machine Translation (NMT). We will learn about the encoder-decoder architecture, which forms the basis of NMT models, and explore attention mechanisms that enable the model to focus on different parts of the input sequence during translation. Through hands-on exercises, we will implement an attention model for English to German translation using TensorFlow, train it on a dataset of sentence pairs, and evaluate its performance. By the end of this week, you will have gained a solid understanding of NMT and its application in language translation."
categories: 
...
![deeplearning.ai](img/logo_deeplearning_ai.png){.column-margin}
...
### How do we learn to **align** two sequences? {#sec-sec-how-to-align-sequences-2}
...
### How do we utilize arbitrary functions on a TRAX Neural Net? {#sec-sec-TRAX-arbitrary-functions}
...
berliner = citizen of berlin berliner = jelly doughnut
...
Another similar method for evaluation is the ROUGE score which calculates precision and recall for machine texts by counting the n-gram overlap between the machine texts and a reference text. Here is an example that calculates recall: ![recall in ROUGE](img/c4w1_screenshot_11.png){.column-margin}
...
The ROUGE-N refers to the overlap of N-grams between the actual system and the reference summaries. Recall and precision can be combined using the [F-score](https://en.wikipedia.org/wiki/F-score) metric.
...
## Minimum Bayes Risk (MBR) {#sec-minimum-bayes-risk}

Minimum Bayes Risk (MBR) Compares many samples against one another. To implement MBR:
...
![MBR](img/c4w1_screenshot_16.png){.column-margin}
...
-   [Exploring the Limits of Transfer Learning with a Unified Text-to-Text Transformer](https://arxiv.org/abs/1910.10683) (Raffel et al, 2019) -​ [Deep contextualized word representations](https://arxiv.org/pdf/1802.05365.pdf) (Peters et al, 2018)
-   [The Illustrated Transformer](http://jalammar.github.io/illustrated-transformer/) (Alammar, 2018)

::: callout-note
add citations for rogue, MBR and Beam search.
:::