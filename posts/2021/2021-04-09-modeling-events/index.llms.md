...
Events are the atoms of a story’s narrative structure. Each bit of dialogue can be viewed as an event, with some extra structure (text, subtext). Exposition is a technique of providing information happening off screen - typically from the back story the history of the characters from before the first event.
...
Notice how in the first sentence there are multiple actions. The boy who is active subject becomes a passive subject as we progress. Also there true subject and object are it and him and need to be resolved. To do this it seems necessary to parse or at least chunk it to find the main and subordinate clause. Setting the issue of chunking/parsing aside the task can be broken down into:

1.  tag the subordinate named entities.
2.  resolve the co-references
3.  assemble different event options.
4.  assign a value to each using a pre-trained model for story value sentiment analysis.
5.  rank the assignment by saliency or using a Bayesian Game
6.  score using a heuristic metric
    1.  favour the verbs in upper levels of the parse tree for the action.
    2.  fitting with prior information
    3.  value assignment requires its own model
7.  revise saliency as future info comes into focus. unless a suitable dataset miraculously makes an appearance some different approaches might be used to produce this.
8.  build a datasets by annotating events like above - the problem is that it is labour intensive and that a bench mark would need to be developed as well.
9.  use a reduced number of actions
10. bootstrap using a heuristic.
11. use frame net to generate frames sentence templates.
    - generate samples.
    - train a siamese model to assign sentences to frames.
12. generate fake events, possibly by sampling events sentences ranked by lower ambiguity or by existing events
13. augment existing events using a word model.
14. build a generator/parser adversarial model scored on perplexity of unsupervised dataset.
...
@online{bochman2021,
  ...
  date = {2021-04-09},
  url = {https://orenbochman.github.io/posts/2021/2021-04-09-modeling-events/},
  langid = {en}
}
```
...
Bochman, Oren. 2021. “Modeling Events.” April 9. <https://orenbochman.github.io/posts/2021/2021-04-09-modeling-events/>.
