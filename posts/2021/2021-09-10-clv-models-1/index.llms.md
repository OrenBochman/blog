[![](chaotic-time-series.jpg)](chaotic-time-series.jpg "Figure 1: customer churn time series")

Figure 1: customer churn time series

> **NOTE:**
>
> [![Buy till you die in a nutshell](../../../images/in_the_nut_shell_coach_retouched.jpg)](../../../images/in_the_nut_shell_coach_retouched.jpg "Buy till you die in a nutshell")
>
...
Many firms interested in estimating customer lifetime value (CLV) This can let understand if a brand is growing, it can help forecast retail sales. It can suggest when and to whom they might intervene with a retention interventions to deter churn. And to some extent it can even help set bounds on acquisition costs for advertising.
...
The paper and model are associated with confusing acronyms. Both the Pareto and negative binomial distributions do not appear directly in the model. Thus the naming conventions needs some explanation.
...
This model is from ([Schmittlein et al. 1987](#ref-Schmittlein1987Counting)) in which the authors propose a framework for understanding customer behavior over time, though primarily they are concerned with counting active customers, which is important in at least three settings:
...
1.  **Purchasing process:**
    ...
    - While this captures heterogeneity in purchasing behavior, it makes an assumption on the timing of purchases: the time between purchases is **exponentially distributed** which has a memoryless property[^1]. This is a simplifying assumption but is not a good fit for bursty purchasing behavior or for periodic purchases (e.g. monthly subscriptions).
...
y\_{i}(t) \mid \lambda_i, \\ \text{alive} \sim \mathcal{Poi}(\lambda_i t) \quad \text{with } \lambda_i \sim \mathcal{Gamma}(r, \alpha) \tag{1}

2.  **Churn process (Exponential-Gamma → Pareto):**

T_i \mid \mu_i \sim \mathcal{Exp}(\mu_i) \quad \text{with } \mu_i \sim \mathcal{Gamma}(s, \beta) \tag{2}

3.  **Joint prior (independence across processes):**

p(\lambda_i, \mu_i) = p(\lambda_i) \\ p(\mu_i)

4.  **Observed data:** For each customer, we observe:
...
5.  **Posterior inference:**
    ...
    - From (x_i, t\_{x,i}, T_i), we update beliefs about whether i is still active.
...
\begin{aligned} \lambda_i &\sim \mathrm{Gamma}(r,\alpha) && \text{(purchase‐rate heterogeneity)}\\ \mu_i &\sim \mathrm{Gamma}(s,\beta) && \text{(churn‐rate heterogeneity)}\\ T_i^{\ast}\mid \mu_i &\sim \mathrm{Exp}(\mu_i) && \text{(latent lifetime)}\\\[2pt\] N_i \mid \lambda_i, T_i^{\ast}, T_i &\sim \text{PoissonProcess}\\\left(\lambda_i\\ \text{on}\\ \[0,\tau_i\]\right),\quad \tau_i=\min(T_i,T_i^{\ast})\\ x_i &= N_i(\tau_i),\qquad t\_{x,i}=\max\\t\le \tau_i:\text{event at }t\\\\ \\ (\text{or }0\text{ if }x_i=0) \end{aligned}

- In the classic BTYD setup, (r,\alpha,s,\beta) are fixed unknowns estimated by MLE; fully Bayesian just adds priors on them.
- Inputs \mathbf{x}\_i=(x_i,t\_{x,i},T_i).
...
[![Plate diagram for the Pareto/NBD](index_files/figure-html/cell-2-output-1.png)](index_files/figure-html/cell-2-output-1.png "Plate diagram for the Pareto/NBD")

Plate diagram for the Pareto/NBD
...
- t\_{x,i}: **recency** — time of the last purchase within the calibration window.
- \mathbf{x}\_i = (x_i,t\_{x,i},T_i): input features used by the model.
...
In ([Fader et al. 2005](#ref-fader2005counting)) the authors developed a new model, the beta-geometric/NBD (BG/NBD), which represents a slight variation in the “behavioral story” associated with the Pareto/NBD, which is much easier to implement, and this has replaced the Pareto/NBD in many applications.
...
Bochman, Oren. 2021. “Customer Lifetime Value - Pareto/NBD (BTYD) Model.” September 14. <https://orenbochman.github.io/posts/2021/2021-09-10-clv-models-1/>.

[^1]: i.e. when the future is independent of the past
