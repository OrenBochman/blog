...
3.  **label shift** - this is where someone renamed a product or a category. Worse they may have completely replaced one web site with another - all product pages have changed and need to be remapped. The solutions are fairly simple - retain the model adding more recent data. Remap old labels to new schema.

### Accuracy

Also if accuracy isn’t impacted by the drift we may not be as worried. Chip Hugen suggests in ([Huyen 2022](#ref-huyen2022designing)) to monitor production accuracy metrics like: *F1 score*, *recall*, *AUC-ROC*, etc. When these are negatively impacted, it is worth noting there are a number of drift we would want to monitor.
...
Bochman, Oren. 2022. “Robust Regression.” September 12. <https://orenbochman.github.io/posts/2022/2022-09-12-robust-regression/>.
