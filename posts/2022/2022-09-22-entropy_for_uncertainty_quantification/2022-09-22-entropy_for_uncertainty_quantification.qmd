...
subtitle: Entropy to quantify uncertainty in probabilistic models
description: using entropy to quantify uncertainty in probabilistic models
category: data science
keywords: [data science, probability, bayes, entropy, uncertainty quantification]
---

There is an e-commerce store. 
Every day it collects the `cost`, `sales` and `shelf price` for each product.
...
$$p
(price) = \frac{1}{days_{tot}} \sum day_{p} 
$$

- The calculation of $P(Q)$ is  

$$p
(demand) = \frac{1}{days_{tot}} \sum day_{q} 
$$
...
It seems like there are some some risk factors when it comes to mistakes in pricing and we should look at mitigating these. 
...
It seems we may be able to do better if we at least acknowledge the uncertainty in our model.
