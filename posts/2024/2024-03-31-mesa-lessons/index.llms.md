...
2.  differential reproduction based on fitness

3.  a fitness function - based on the expected progeny of an agent.

4.  This can be based recusively by a linear model of intrinsics like high vision and low metabolism a smaller initial endowment the total amount of resources an agent has collected thier age the number of progeny they have - and the thier fitness.
...
3.  Agent having private information, and forming coalitions. Both concepts are discussed in to text but not implemented. The first in the context of more realistic trade rules. The latter in the context of paying agents to reduce harvesting to avoid pollution.
...
1.  the ability to go to any of the maximal sugar points in the field of view creates a nomadic type of individual.

2.  there was a bug in mesa’s randomization which led to near cyclic motion for agents at the edge of the sugar contours. Periodic motion is a form of teratorilaism which could be good for avoiding desiases and may make it easier to mate with other nearby teritorial agents. The idea here is to implement an exploter agent that prefers to go and exploit places it visted before if they are maximal. This can lead to near periodic motion. Is this a form of chaos. Also we may give it a epsilon chance to explore which may let it discover nearby maxima.

3.  A friend suggested letting agents leave phenome tracks. These can form the basis of a signaling system.
...
if agents can signal using a pheromone trail they could lay a path from higher levels of resources to a nest…. (there might be unique pheromones for each resource or a more complex signal using combination of multiple pheromones to encode each resource) a weak diffusion rule would create a gradient to lead to the agent or to the resource.

## The SugarScape Trade rule issues

1.  the rule is unfair.

- While the Edgeworth box has pareto-efficient solutions where indifference curves are at a tangent and therefore the MRS which corresponding to prices for trade are in agreement. Also there is only one such point for each pair of curves if they have a common point. Also there is one point where both utilities are highest. This seems to be the best trade to consider.

  The trade algorithms uses points where the two curves intersect not tangents. However each indifference curve probably intersect with all the other traders curves. Some may have more curves in the box (their utility levels are packed closer) Others less. Without some mechanism to sort these out we cant be certain that bargaining steps are sensible. The problem is that agents are not really indifferent to all the combination of sugar and spice - at least in the sense that under a certain amount of sugar or spice they will be wealthy on paper but likely starve in the next time step. The algorithm leads to more on diagonal solutions but what if I am very sugar rich. I may trade 1/N of my excess sugar with each neighbor for one unit of spice. But one unit of spice from each may mean they have less then their metabolism and they will starve before they can make a trade or harvest ay spice.

  One agent may have more viable curves than the other. Ideally we should be picking two curves with similar utility levels…. The intersection points can represent radically different pricing levels, particularly for an agent with a highly imbalanced endowment. The pareto point leads to a point where both MRS are equal and the resulting endowments are better.
  ...
  So on this issue we should pick a point on the indifference curve that at a utility level that at least won’t starve out trade partner or ourselves.

  This mean clipping the indifference curves at the metabolic minimum.

- there is no money and trades are in integral units and determine the trade amount

- the trade amount are unfair because the prices are unfair and because one side is pegged to 1.

- All this is moot if the alg can reach the same pareto optimal solution by small steps. But I think that its unlikely. In the trade because the steps are integral and one side is pegged to one there are round off errors. With every trade some value is lost (on one or both sides).
...
2.  trade is inefficient.

- trade always use 1 unit for one of the agents.
- to trade 3 for 3 3 trades are needed.

3.  trade increases utility but can make an agent worse off.

- after a trade a fertile agent can become wealthier but infertile if they are reduced below a multiple of their endowment.
- after a trade an agent can be wealthier in utility but not in survivability so they may have an extra unit of sugar they don’t need but not enough spice so they might starve.

4.  agents that have participate in trade traded will generally
5.  a smarter agent who can pick his trade partners and their order can do them some damage

## Better trade rules:

A fundamental question is this:

If locally markets are heterogeneous and weakly interacting can we be sure of the globally market reaching an reaching an equilibrium between supply and demand? It there are many possible equilibria how long to reach one and if not what other can we say for the long term, i.e. is there a local and global steady state where supply and demand meet or can we have other outcomes like periodic, chaotic, and random states globally.
...
- Is the trade rule really inefficient ?
...
- What is in the way of equilibrium formation ?
  ...
  - inability or inefficiency of making trades across market boundaries
...
The down side of an inefficient trade rule is that it may lead to less efficient market. The market in SC can be viewed as locally fragmented or if there are enough agents it may be a collection of weakly interacting markets.

Intuitively

Agents with lower vision tend to persist at the local market edges and become inefficient conduits of trade in the trade network instead of finding strategic locations where they could maximize the benefit of arbitrage between local markets.

### Arbitraders

We can explore this by letting some agents who are less efficient harvesters have a movement rule that lets them seek out such locations with maximal arbitrage option. If there landscape is made of K blobs of different resources there may be greater need for such traders, and As more regions

### Super-traders

We can setup a super-trader that is basically a supermarket in each neighborhood. These agents with better vision that don’t have a metabolism cost or harvesting step.

They can see each other and trade with each other allowing for greater efficiency in the market.

We can rerun markets with or without super-traders and with super-traders with more vision. Their wealth distribution could be a way to track the lost benefits due to vision fragmentation.

### Edgeworth box with pareto-efficient trades maximizing utility objective
...
1.  setup the edgeworth box with
2.  store the pareto-optimal solution that maximises both agents’ utilities and is viable for both agents
3.  do the trade with the highest gain in utility to me.
4.  remove agent from neighbours
5.  my preferences have changed so return to 1

### Edgeworth box with pareto-efficient trades for survivability objective

we want to restrict solutions to trades that increase the survivability this is like a quantum constraint on the utility.

1.  are the any solutions ?
2.  what is the best solution - and what is the closest option.

### Edgeworth box with pareto-efficient trades for hybrid objective

1.  we want a multiterm utility for
2.  survivability of n rounds Quant(U(M))
3.  reproduction endowments for up to 4 offsprings U(E)
4.  trades for U(M) that do not reduce the above two utility components
5.  side trades that reduce pollutions ?

the tricky part here is to define an hybrid utilities and solve its edgeworth box as we add more rules we arrive at more complex utilities.

So we may need to rethink a simpler way to find acceptable trades….

## Gale-Shapley Trade partner allocation

A second idea - based on [Gale–Shapley](https://en.wikipedia.org/wiki/Gale%E2%80%93Shapley_algorithm) algorithm for Stable marriage within neighborhoods

1.  Each agent should find their preferred trades within their FOV.
...
For this variant for ABM we want to impose two restrictions on marriages: 1. offers made are restricted by myopia to the agents FOV. 2. offers received are restricted locally to the agents Max_Vision FOV.
...
1.  A set of Agent contributes (resources) or money to partnership at some price becoming an share owner of this company.

2.  agents can be silent partners of active members acting on behalf of company

3.  providing information on market conditions, supply, (by agent and on ground) demand, prices.

4.  Getting and receiving trade offers.

5.  Making localized decisions on behalf of the company (possibly subject to share majority in a neighborhood)

6.  Once the company has traded agents can contribute/withdraw resources with ownership adjustments

7.  the company can be dissolved after n-time steps, if enough agents migrate, or if the inventory is too low

8.  The partnership has no Metabolism so it will need a different utility function - (this can be ml based based on info from agents).

9.  members of a company may use an information utility to deploy to locations that maximize information….

10. will agents have shares in multiple companies. (perhaps conflict of interest can force agent to be a silent partner in the cooperations they are not active in)

11. can agents get rewards for bringing in trades

12. can agents get loans from companies

13. will comparative advantage let certain agent gain control of such companies
...
2.  We want to find a maximally parsimonious rule for which the effect emerges
...
@online{bochman2024,
  ...
  title = {Mesa},
  date = {2024-03-31},
  url = {https://orenbochman.github.io/posts/2024/2024-03-31-mesa-lessons/},
  langid = {en}
}
```
...
Bochman, Oren. 2024. “Mesa.” March 31. <https://orenbochman.github.io/posts/2024/2024-03-31-mesa-lessons/>.
