...
date: 2024-03-31
title: "Mesa"
subtitle: "Lessons from implementing Sugarscape in Mesa"
description: "In this post I share some of the lessons I learned from implementing Sugarscape in Mesa. I cover the design decisions I made and how they helped me to overcome some of the challenges I faced."
image: /images/cover.png
categories: [MESA, agent based modeling, sugarscape]
---
...
if agents can signal using a pheromone trail they could lay a path from higher levels of resources to a nest....
(there might be unique pheromones for each resource or a more complex signal using combination of multiple pheromones to encode each resource)
...
1. the rule is unfair.
  - While the Edgeworth box has pareto-efficient solutions where indifference 
    ...
    However each indifference curve probably intersect with all the other traders 
    ...
    certain that bargaining steps are sensible. The problem is that agents are
    not really indifferent to all the combination of sugar and spice - at least 
    ...
    my excess sugar with each neighbor for one unit of spice. 
    But one unit of spice from each may mean they have less then their metabolism 
    and they will starve before they can make a trade or harvest ay spice.
    
    One agent may have more viable curves than the other. Ideally we should be 
    picking two curves with similar utility levels.... The intersection points
    can represent radically different pricing levels, particularly for an agent 
    with a highly imbalanced endowment. The pareto point leads to a point where
    ...
    So on this issue we should pick a point on the indifference curve that at 
    a utility level that at least won't starve out trade partner or ourselves.
  ...
  are integral and one side is pegged to one there are round off errors. With 
 ...
 - perhaps in view of this agents should be allowed to refuse unless they get a better
   ...
   several rounds in each neighborhood for matching traders.
...
3. trade increases utility but can make an agent worse off.
  - after a trade a fertile agent can become wealthier but infertile if they are 
    reduced below a multiple of their endowment.
  - after a trade an agent can be wealthier in utility but not in survivability so
    they may have an extra unit of sugar they don't need but not enough spice so they
    might starve.
4. agents that have participate in trade traded will generally 
4. a smarter agent who can pick his trade partners and their order can do them some
   damage
...
If locally markets are heterogeneous and weakly interacting can we be sure of the 
globally market reaching an reaching an equilibrium between supply and demand?
It there are many possible equilibria how long to reach one and if not what
...
chaotic, and random states globally.
...
The down side of an inefficient trade rule is that it may lead to less efficient market.
The market in SC can be viewed as locally fragmented or if there are enough agents it
may be a collection of weakly interacting markets. 

Intuitively

Agents with lower vision tend to persist at the local market edges and become inefficient conduits of trade in the trade network instead of finding strategic locations where they
could maximize the benefit of arbitrage between local markets. 


### Arbitraders 

We can explore this by letting some agents who are less efficient harvesters have a 
...
We can setup a super-trader that is basically a supermarket in each neighborhood.
These agents with better vision that don't have a metabolism cost or harvesting step.

They can see each other and trade with each other allowing for greater efficiency in
the market. 

We can rerun markets with or without super-traders and with super-traders with more 
vision. Their wealth distribution could be a way to track the lost benefits due 
...
1. for each neighbour in neighbours:
  ...
  d. my preferences have changed so return to 1
  
### Edgeworth box with pareto-efficient trades for survivability objective

we want to restrict solutions to trades that increase the survivability
this is like a quantum constraint on the utility.
...
1. we want a multiterm utility for
  ...
  3. trades for U(M) that do not reduce the above two utility components 
  4. side trades that reduce pollutions ?
  
the tricky part here is to define an hybrid utilities and solve its edgeworth box
...
1. offers made are restricted by myopia to the agents FOV.
2. offers received are restricted locally to the agents Max_Vision FOV.  
...
1. will agents have shares in multiple companies. (perhaps conflict of interest can force agent to be a silent partner in the cooperations they are not active in)
