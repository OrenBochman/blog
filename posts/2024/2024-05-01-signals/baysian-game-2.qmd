...
First has to do with initilizing the algorithm in some optimal way. Like
the battle of the sexes there is no easy way to initialize the algorithm
unless the agents can coordinate on a single equilibrium. If the state
...
explote the Gittin's index from [@sutton2018reinforcement] I had already
...
idea regarding the prior. I had a fairly efficient algorithm for the
...
More so I was coming to terms that I could likely come up with Bayesian
...
transpositions that cancel each other out. A transposition is a very
...
can define a prior over permutations or general on any group.

Another point in favor of transpositions is that they have one
operation, their composition just a product and since probabilities are
