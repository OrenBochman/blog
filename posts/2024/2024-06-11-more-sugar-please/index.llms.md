...
In ([Bak et al. 1988](#ref-Bak1988SelforganizedC)) the authors introduced the [abelian sandpile model](https://en.wikipedia.org/wiki/Abelian_sandpile_model). They showed that a simple model of sandpile could generate complex landscapes.

We could incorporate this model to generate the landscape of sugarscape. In this version region with sugar are created by dropping sugar on the landscape. The sugar will pile up in some regions and create hills and valleys. The agents will then need to navigate this complex landscape to gather resources. If they are not careful they might fall into a valley and be unable to get out. If they are unlucky they might be caught in a landslide and be buried in sugar.

### Xland Sugarscape

The paper ([Team et al. 2021](#ref-Team2021OpenEndedLL)) described a new approach to learning called Open-Ended Learning. In it an functional generated environment was described called Xland. I think that one way to make sugarscape more more challenging is by composing more complex landscapes.

Xland uses Quantum Wave Collapse algorithm to generate 3d landscapes. This is a type of procedural generation.

When a the agents needs to navigate a complex landscape to gather resources survival becomes more challenging. This in can lead to more importance for trade between agent and even between communities. If things get really tough agents might prefer combat to trade.
...
- We can model this using a predator-prey models The different biomes have animals in a predator-prey relationship. They keep the population of each other in check and husband the resources of the land.
  ...
  Thier populations are governed by Lekrotta-Volterra differential equations.

  Such Pairs might be:

  - For glaciers: Yetis and penguins
  ...
  As one is hunted e.g. stag hunt targeting deer will boost the numbers of rabbits if the number of deer is too small they might be decimated by wolves and the rabbits. One of both of these may emerge from thier habitat and attack the agents or thier crops.
  ...
  - Cutting down trees might lead to deforestation, erosion and desertification.
    ...
    - this would eliminate the salmon and enrages the bears.
    ...
    - desertification would reduce the carrying capacity of the land by reducing max_growth to
...
- Hunting can also lead to burst in the growth of undesired species that will target crops, attack their agents and spread disease.
...
- Nature could be made less and less forgiving to our agents.
  ...
  - droughts, fires, floods, hurricanes, tsunamis, earthquakes, avalanches, mudslides, etc.
  - plagues of locusts, rats, bats, that eat crops if their predators are killed.
...
- Mountains of high elevation might have snow that generates a source of water.
...
- Agents might be able to build canals - these might together with boats would allow more efficient drainage and trading with boats along the canal, a public goods game. If there are many resources a canal system might be needed to allow agents trade efficiently so as to survive
...
- After a flood Agents might need to cooperate to drain an area to restore land to harvest it to full capacity.
  ...
  - Draining will require a certain amount of time. At this time free loaders may use the cleared land to harvest resources. Ideally, though the agents should switch roles or better yet share the resources harvested in some way while draining is going on.
  - We can complicate this by making the draining move water from one cell to another until it is pushed to a lower elevation. More agent participate they can drain the area faster. This is a stag hunt game. However as more areas are drained agents might prefer/need to harvest.
...
- we may tiw behavior to kinship groups via terms in the utility function that are shared by members of the group.

### Social capital, Reputation & Prestige comes to Sugarscape
...
Social trust  
this is a the trust between two agents. Agents that have not interacted have no ties. Agents that have positive interactions have strong positive ties Agents that gave negative interactions have strong negative ties. Social trust increases the chance of future cooperation between agents. (For a trade, a public good game, etc an agent will prioritize cooperate with agents they trust.) Trust might become an element of the utility function controlling movement.
...
- Complex dilemma tasks:
  - We may introduce complex tasks that require coordination between many agent for the group to survive and thrive.
  - a flood might require agents to drain an area to restore land to harvest it to full capacity.
  - an avalanche might require agents to dig out a buried agent quickly.
  - a fire might require agents to build a fire break to save their crops.
  - a war might require agents to coordinate to defeat a stronger enemy.
  - a shortage of resources might require agents to coordinate to share resources.
  - group trades might be required when the group exceeds the carrying capacity of the land.
  - a public good might be required to increase the carrying capacity of the land.
  - a boat might need three agents to operate.
  - two rowers and a time keeper.
  - access to certain resources might require fabricating tools artifacts by specialists.
  - in a dragon slaying quest a leader might need to mobilize a team of specialists
  - some wealthy patron to finance the quest
  - a sword smith, a armorer, to outfit fighters
  - a ranger to find the layer,
  - k pages to locate keys to open the doors to the dragon’s lair
  - a knight access the dragon’s lair and then fight it.
  - each time step the dragon might return and attack a tribe member or cause a fire, reducing the tribe’s carrying capacity.
  - in games of environmental cleanup agents might need to coordinate to most effectively harvest and clean up their environment.

Thus in general being right isn’t enough, an effective leader must be able to marshal resources and mobilize a team of allies to get the job done. The agents would also need to plan to get the task done. This would require a more complex decision making process

So we might have simulation where agents just focus on one complex task like in the mini-grid environment. Then one might add bridge building curriculum learning, bridge building, cleanup, stag hunt, etc.

Complex tasks might require coordination between coalitions of agents. More so when the tasks might also require access mobilizing a diverse network of resources, by specialist agents then we can see how social capital might be beneficial.
...
- leaders with the most prestige should be able to make decisions that affect the group. Ideally these decision should be made locally in a way that the that the group can only achieve the goal and get the benefits by working together. This can allow the benefits of having a leader.

- wolfpack stag hunt mini-games. agents must form a wolfpack to hunt a stag.
...
- Autocracy where the strongest agent becomes the leader.
...
- Democracy where agents vote on group decisions. This could be a simple majority vote.
  ...
  - agents vote are a function of friendship ties, kinship ties and social prestige.
...
### Specialization comes to Sugarscape - Specialization I

- each agent has either a probability of success or an output level associated with thier abilities.
- agents who specialize can generate greater wealth and then trade for the missing commodities.
- agents can also learn to trade thier specialized goods for other goods.

### Art comes to Sugarscape - Specialization II
...
### Artifacts comes to sugarscape - Specialization III
...
- agents can create artifacts that can be traded
  ...
  - artifacts may confer an intrinsic bonus
...
Team, Open-Ended Learning, Adam Stooke, Anuj Mahajan, et al. 2021. “Open-Ended Learning Leads to Generally Capable Agents.” *ArXiv* abs/2107.12808.
...
Bochman, Oren. 2024. “More Sugar Please.” June 11. <https://orenbochman.github.io/posts/2024/2024-06-11-more-sugar-please/>.
