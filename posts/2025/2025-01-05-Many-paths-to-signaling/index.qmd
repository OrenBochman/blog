...
A related issues is that of enumerating different types of equilibriums in larger games. For basic lewis signaling games this is not very difficult but once one imposes a structure on the state and requires complex signals to emerge we get to a point where it may be quite challenging to enumerate all the possible equilibria.
