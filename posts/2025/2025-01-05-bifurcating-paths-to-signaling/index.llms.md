...
Another point of interest to me is to consider the emergence of grammar and of a morphology. In ([Nowak and Krakauer 1999](#ref-nowak1999evolution)) The authors give a result for the emergence of grammar in a signaling system. This is that there are many more
...
[![lewis signaling game](./lewis_extensive_form.svg)](./lewis_extensive_form.svg "lewis signaling game")

lewis signaling game

In The book signals ([Skyrms 2010a](#ref-skyrms2010signals)) the author, Skyrms, discusses how Lewis challenged the skepticism of his advisor Quine regarding the meaning and convention may arise via an arbitrary mechanism like symmetry breaking.
...
However this section is based on ([Huttegger et al. 2014](#ref-huttegger2014some))
...
4.  Source coding scenario errors in encoding & decoding - based on paper by (Nowak and Krakauer)

5.  errors in the transmission channel based on paper by (Nowak and Krakauer)

6.  risks - there are signals with monotonically increasing risk.
...
1.  They have booth been observing the state space long enough to infer the distribution of states to a high degree of confidence. **coordinate via the state distribution**
2.  They can listen to a third party who knows the distribution and learn to signal from them. **coordinate by imitation**
3.  They can access a state classifier and send it random noise thus deriving an empirical distribution of states in the classifier (not nature) and use it to learn the signaling system. **coordinate via a classifier**
...
- The Expected number of steps \begin{aligned} \mathbb{E}\[steps\] &= \sum\_{k=1}^{N} \frac{1}{P\_{\text{success k}}} \times P\_\text{failure up to k} \newline &= \sum\_{k=1}^{N} \frac{1}{{N-(k-1)}} \underbrace{\times \prod\_{i=1}^{k-1} \frac{N-i}{N-i+1}}\_{\text{telescopic product}} \newline &= \sum\_{k=1}^{N} \frac{1}{\cancel{{N-(k-1)}}} \times \frac{\cancel{{N-(k-1)}}}{N} \newline \end{aligned}
...
>
> C = \sum\_{i=1}^{N} \sum\_{j=1}^{M} \delta\_{ij} \\ \tag{1}
>
> where \delta\_{ij} is 1 if the signal j is different from the target signal for state i and 0 otherwise.
...
This is an idea is similar to the approach used by ([Nowak 2006](#ref-Nowak2006Evolutionary)) on errors in signaling. However in this case we are more interested in the case where there are adversarial agents who are trying to deceive the receivers and the problem of these receivers to learn a signaling system that is robust to adversarial agents
...
e.g. in the battle of the sexes we should expect perfect rewards e.g. in a three way traffic junction game we might expect the agents to signal thier intentions to turn left or right and to go straight. This would allow them to avoid accidents. One such mechanism might be a game of paper rock scissors to determine the priority of the agents. e.g. in the [Braess’s Paradox](https://en.wikipedia.org/wiki/Braess%27s_paradox) establishment a high way though a city we might end up increasing the traffic jams.

## Co-adaptaion and Semantic Drift and grounding.

c.f. ([Rita et al. 2022](#ref-rita2022emergent)) paper of co-adaptation loss! c.f. ([(FAIR)† et al. 2022](#ref-meta2022human)) solving the game of diplomacy c.f. ([Barrett and Skyrms 2017](#ref-barrett2017self)) on templates by skryms
...
2.  learning simple models first using `easy` examples
3.  infer the prior that best supports the inductive bias for each subtask
4.  assemble these into deeper hierarchies to combine earlier learning into more complex structures.
5.  check if these nested models can be used to learn
...
2.  Agents need to coordinate on one of an infinite set of hypotheses (which coin C\_\theta, \theta \in\[0,1\] generates a sequence of states S=\[H\|T\]+).
3.  Agents need to coordinate on one of two hypothesis of different complexity. E.g. similar to the previous scenario but
    - H0 \theta \in \[0,1\]
    - H0 \theta \in \[0.45,.55\] Which requires penalizing the more flexible model for it’s complexity!
4.  agents should learn semantic heirarchies for the language (the fixed number of categories corresponds to a derisclet prior over the states)
5.  agent should learn morphological categories for the words
6.  agent should learn to describe properties of objects say animals or plants encoded in a feature vector using nouns and adjectives. (Two word state)
7.  agent should learn Propositional Logic over binary features over some arbitrary number
8.  agent should be able to learn to parse and evaluate simple arithmetic expressions possibly nested
...
2.  
3.  
4.  https://videolectures.net/videos/icml07_tenenbaum_bmhi
5.  “Bayesian models of cognition” chapter in Handbook of Computational Psychology
6.  https://cocosci.princeton.edu/tom/bayes.html
...
2.  Also this creates a new benefit that takes the form of social welfare. This means that derelict members of the watch or those that raise false alarms will be subject to punishment by thier peers. This is a form of **costly signaling** that is used to maintain the integrity of the tribe.

3.  Less capable forgers will be able to more quickly learn the locations of food sources by sharing signals of the more experienced foragers whenever they return to the safety of the tribe. This may also allow the tribe to better exploit the food sources that are available to them in a far larger area. Thus the carrying capacity of the tribe is increased by sharing of information, and the tribe might also become more localized and less nomadic. They may choose to migrate to new areas only when the food sources are depleted in a larger neighborhood.

4.  Stag hunts - hunting large game that requires coordination of multiple hunters, the ability to signal and coordinate the hunt using this new signaling system may allow the tribe to hunt larger game that they would not have been able to hunt before. This will also let them specialize in hunting.
...
(FAIR)†, Meta Fundamental AI Research Diplomacy Team, Anton Bakhtin, Noam Brown, et al. 2022. “Human-Level Play in the Game of Diplomacy by Combining Language Models with Strategic Reasoning.” *Science* 378 (6624): 1067–74.
...
Lewis, David Kellogg. 1969. *Convention: A Philosophical Study*. Wiley-Blackwell.
...
Skyrms, Brian. 2010a. “14512 Complex Signals and Compositionality.” In *Signals: Evolution, Learning, and Information*. Oxford University Press. <https://doi.org/10.1093/acprof:oso/9780199580828.003.0013>.

Skyrms, Brian. 2010b. “14512 Complex Signals and Compositionality.” In *Signals: Evolution, Learning, and Information*. Oxford University Press. <https://doi.org/10.1093/acprof:oso/9780199580828.003.0013>.
...
Bochman, Oren. 2025. “The Many Path To A Signaling System.” March 24. <https://orenbochman.github.io/posts/2025/2025-01-05-bifurcating-paths-to-signaling/>.
