...
* **Demo result:** On a New York City taxi and rideshare ETL workload with about **44 million rows**, regular pandas took roughly **60–70 seconds**, while Bodo DataFrames ran in about **7 seconds** on four Mac cores, giving around a **10× speedup**.
...
* **Execution model:** Bodo uses **single program, multiple data (SPMD)** execution rather than the usual driver-executor model used by systems like Spark or Dask. Workers run the same program on different data chunks and communicate directly using **message passing interface (MPI)**, which reduces driver bottlenecks.
...
:::: {.sl}
::: {.sl-text}
* **Lazy evaluation and optimization:** Bodo does not immediately execute each pandas operation. 
:::
::: {.sl-fig}
...
- Don't these other libraries also promis to be drop in replacements for pandas ? 
