---
title: "TinyTroupe: Enhancing Marketing Insights through LLM-Powered Multiagent Persona Simulation"
subtitle: "PyData Global 2025 Recap"
date: 2025-12-12
categories: [pydata]
keywords: [pydata, odsc, Transformers, Deep Learning, PyTorch, Machine Learning]
...
takechanman1228)
- [linkedin](https://www.linkedin.com/in/hajime-takeda/)
:::
...
Lots of the early propmt engineering tricks peple shared were along the lines of "You are X, a person who does Y and thinks Z". 
This is fine for a superficial session but the chat bot quickly drift away to its own personality.
...
::: {.column-margin}
{{< video https://youtu.be/IAVgFr30QrE >}}
...
5. Another approach is based on using  reinforcement learning approach to keep the PC on track. (Exploration v.s. Exploitation trade off). 
   In the following sense: 
     a. Exploitation: draw from the personality's corpus when there is a good quote or opinion in the RAG and then reuse it when appropriate. 
     b. Exploration: let the PC use the LLM's default response when RAG does not yield good results or is out of distribution.
   The rub is that we need a reward or utility as feedback to the model to let it know how well it is doing. One idea is to use sentiment analysis to see if the response is useful or not.
   However not all is lost - if we follow the Mike Ferry he suggest that we can classify personality by prequalifing using a short survey. 
