...
::: {.callout-note}
## TL;DR - Too Long; Didn't Read about Costly Signaling <!-- Short Catchy title -->
...
One place where costly signaling makes sense is email-spam. If signaling has no cost this would lead any agent that can benefit by signals to do so. This drowns out the other signals. This suggests a social costs of cheap talk. If there is a penelty for sending spam, the spammer would only send spam where the expected benefits outweigh the costs. If the expected benefit is  usd 1/1,000,000 we could charge 1 cent per email and the spammer would not send such an spam email. However if they could get sufficently high payoffs they might choose to still send spam. 
